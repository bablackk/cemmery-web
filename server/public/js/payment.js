...
function updatePaymentUI() {
  ...
  for (const [key, value] of items.entries()) {
    ...
    payment.innerHTML = `
      ...
      <div class="properties">
        ...
        <div class="price-properties"><span>${value.price}đ</span>
        </div>
      </div>
      <input type="text" name="productOrder" style="display: none" value="${value.name}">
      <input type="text" name="size" style="display: none" value=${value.size}>
      <input type="text" name="quantity" style="display: none" value=${value.quantity}>
  ...
  if (count >= 0) {
    ...
    const inputTotal = document.querySelector(".input-totalMoney");
    if (total >= 500000) {
